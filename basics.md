...
## Структура хранения материалов при работе в Adobe InDesign
...
2. Всегда лучше делать бэкапы. QGIS, к сожалению, не самое надёжное ПО, поэтому нередки случаи регулярных крашей проекта. Чтобы избежать потери данных, рекомендуется использовать [плагин](https://plugins.qgis.org/plugins/autoSaver/#plugin-details) для автоматического сохранения;  
3. Файлы должны быть названы так, чтобы любой человек мог понять, что в них хранится;  
4. В названиях файлов и папок, а также в пути к проекту лучше не использовать кириллические символы и пробелы;  
5. Лучше хранить данные в формате GeoPackage, при этом важно корректно и понятно называть слои (см. п. 3 и п. 4). У шейпфайлов достаточно много [недостатков](http://switchfromshapefile.org/), поэтому их лучше не использовать.   
...
6. Нельзя забывать про одно из важнейших [правил совместной работы с файлами и данными](#Правило-бойскаутов).